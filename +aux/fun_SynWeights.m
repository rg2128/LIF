ni_e% function [J, params]=SynWeights(params)
%
% OUTPUT
%       J                =synaptic matrix
...
%
jee=Jee;
jee_out=Jminus*Jee; % inter-cluster potentiation
jee_in=Jplus*Jee; % intra-cluster depression
jei=-Jei;
jie=Jie;
jii=-Jii;
...
fprintf('  --- fraction of bg Exc units: %0.03g',N_e/Numbg);
switch Network.clust
    ...
    case 'het'
        ...
        while (sum(Nc)-(N_e-Numbg))~=0 || any(Nc<0)         % To randomize excitatory cells per cluster and keep number 1440 (80% of toal -10% extra(Out of cluster))
...
% JEE2 = (jee*(ones(N_e)+delta*randn(N_e,N_e)));
JEE=(jee*(ones(N_e)+delta*randn(N_e,N_e))).*(rand([N_e,N_e])<peeout); %Adding std*normal distribution of mean 0 to weights and making it sparse
if strcmp(Network.clust,'het') || strcmp(Network.clust,'hom')
    %%First we assigned all the weights and made them 20% now we add
    %%cluster specific information with J+ and J-
...
if strcmp(Network.clust,'het')
    ...
    fprintf('  --- clusters size: mean=%0.03g, sd/mean=%0.03g\n',mean(a),std(a)/mean(a));
end
