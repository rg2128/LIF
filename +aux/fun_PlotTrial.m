function[Trace] =  fun_PlotTrial(data,firings,Params,J,imagecount,Tone,breathgain,inhgain,copies)
...
plot_currents=0; % postsynaptic current for representative E and I neurons
plot_rates=1; % average cluster firing rates
plot_stimulus=0; % stimuli time course

% load simulation results
Params.Sim.plot_length=Params.Sim.plot_length;
ExcCells = Params.Stimulus.feat(1).selective;
% save plots
...
if plot_raster
    if any(any(firings))
        ...
        PlotAllNeurons(Ext,firings,IN,imagecount,Tone,breathgain,inhgain,copies);
...
if plot_rates
    if p>1 && any(any(firings))
        ...
        % A. Cluster PSTH
        %         subplot(2,1,1)
        [rate, x_bins,Trace]=PlotPopRates(bins,firings,N_e,N_i,NcE,J,imagecount,ExcCells,Tone,breathgain,inhgain,copies);
%         xlim(XLIM);
...
if plot_stimulus
    if any(strcmp(fieldnames(Ext),'stim'))
        ...
        if any(strcmp(Params.stimuli,'CS'))
            figure;
            h=[];  numfig=numfig+1; hold on;
